...
* **Key Hyperparameters:**
  ...
  * `batch_size`: The larger it is, the more stable the gradients. Heavily limited by GPU VRAM.
...
* **Typical Loss Function:** Categorical Cross-Entropy (Multi-class Classification), BCE (Multi-label).
...
* **NO-GO (Avoid when):** Edge devices with power limits, need for aggressive real-time, low VRAM availability.
...
**[SUPERVISED] [2D VISION] [CLASSIFICATION / SOTA BACKBONE]**
...
* **NO-GO (Avoid when):** Small or medium datasets. Limited compute resources (no GPU cluster).
...
* **Common Pitfalls & Quick Fixes:**
  * **Pitfall:** A ViT trained from scratch on a custom small dataset performs embarrassingly, being beaten by a simple ResNet-18.
    * *Quick Fix:* ViTs do not have the innate geometric knowledge of CNNs (Inductive Bias — the idea that nearby pixels are correlated). If you have few data, **YOU MUST use pre-trained weights (Transfer Learning)** from massive datasets (ImageNet-21k, JFT-300M). Never train a ViT from scratch outside a Big Tech research hub.
