...
* **NO-GO (Avoid when):** Native spatial data (images -> use CNN) or sequential data (text/audio -> use RNN/Transformer). Small/medium tabular datasets (XGBoost dominates MLP in performance and setup time).
...
| Highly scalable: GPU training processes huge mini-batches. | Data hungry: overfits instantly on small datasets. |
...
* **Typical Loss Function:** MSE/MAE (Regression); Categorical Cross-Entropy (Multi-class Classification); Binary Cross-Entropy (Binary Classification).
